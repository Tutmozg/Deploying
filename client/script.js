...
req1.addEventListener('click', async () => {
    try {
        ...
        const users = await getData('https://lesson-task-06-04-2024.onrender.com/getUsers')
...
req2.addEventListener('click', async () => {
    ...
    try {
        ...
        console.log(user)
        let info = await postData('https://lesson-task-06-04-2024.onrender.com/addUser', user)
        const users = await getData('https://lesson-task-06-04-2024.onrender.com/getUsers')
        users.users.forEach(el => {
            container1.insertAdjacentHTML('beforeend', `
            <div class="product">
...
req4.addEventListener('click', async () => {
    try {
        ...
        await patchData(`https://lesson-task-06-04-2024.onrender.com/editUser/${newName}`, user)
        const users = await getData('https://lesson-task-06-04-2024.onrender.com/getUsers')
        users.users.forEach(el => {
            if (el.salary != null) {
                container2.insertAdjacentHTML('beforeend', `
